    ...
    public void updateRPC(String state, String[] serverName) {
        try {
            if (this.rpcClient != null) {
                ...
                long epochSeconds = OffsetDateTime.now().toEpochSecond();
                var8.setState(state).setDetails(serverName[2]).setStartTimestamp(epochSeconds).setLargeImage(serverName[1], serverName[0]);
